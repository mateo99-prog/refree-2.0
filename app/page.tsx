...
const normalizeRateField = (value: string) => value.trim().replace(/\s+/g, " ").toLocaleLowerCase("es-ES");
...
function MatchDialog({ rates, onAdd }: { rates: Rate[]; onAdd: (m: Match) => void }) {
  ...
  const submit = () => { if (!form.home.trim() || !form.away.trim()) return; const match: Match = { id: makeId(), ...form, category: form.category.trim() || "Sin categoría", role: form.role.trim() || "Árbitro", gross: Math.max(0, Number(form.gross) || 0), diets: 0, retention: RETENTION_RATE, partners: [], video: false, status: "confirmado" }; onAdd(applySavedRate(match, rates)); setRateMessage(""); setOpen(false); };
...
export default function RefFlow() {
  ...
  const createDraft = () => { const date = pdfText.match(/\b(\d{1,2})[\/-](\d{1,2})[\/-](20\d{2})\b/); const time = pdfText.match(/\b([01]?\d|2[0-3]):([0-5]\d)\b/); const d = date ? `${date[3]}-${date[2].padStart(2, "0")}-${date[1].padStart(2, "0")}` : "2026-09-26"; setData((old) => { const draft: Match = { id: makeId(), date: d, time: time?.[0] || "18:00", home: "Revisar equipo local", away: "Revisar equipo visitante", category: "Pendiente de calibrar", role: "Árbitro", venue: "Revisar pabellón", gross: 0, diets: 0, retention: RETENTION_RATE, partners: [], video: false, status: "pendiente" }; return { ...old, matches: [applySavedRate(draft, old.rates), ...old.matches] }; }); setView("partidos"); setNotice("He creado un borrador. Revísalo: al completar categoría y función se aplicará la tarifa guardada que coincida."); };
